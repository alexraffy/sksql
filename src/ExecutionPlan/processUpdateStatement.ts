...
export function processUpdateStatement(context: TExecutionContext, statement: TQueryUpdate) {

    let update = statement as TQueryUpdate;

    let numberOfRowsModified: number = 0;
    let done = false;
    let tables = openTables(statement);
    let newContext: TExecutionContext = JSON.parse(JSON.stringify(context));
    newContext.openTables = tables;

    let planDescription = "";
    let plan: TEP[] = generateExecutionPlanFromStatement(context, statement);
    ...
    runUpdatePlan(context, statement, plan);
