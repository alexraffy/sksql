...
const keyWords = ["ADD", "EXTERNAL", "PROCEDURE", "ALL", "FETCH" , "PUBLIC", "ALTER", "FILE", "RAISERROR", "AND", "FILLFACTOR",
    ...
    "SHUTDOWN", "CONTINUE", "KILL", "SOME", "CONVERT", "LEFT", "STATISTICS", "CREATE", "LIKE", "SYSTEM_USER",
    "CROSS", "LINENO", "TABLE", "CURRENT", "LOAD", "TABLESAMPLE", "CURRENT_DATE", "MERGE", "TEXTSIZE",
    "CURRENT_TIME", "NATIONAL", "THEN", "CURRENT_TIMESTAMP", "NOCHECK", "TO", "CURRENT_USER", "NONCLUSTERED",
...
export function isKeyword(input: string): boolean {
    if (typeof input !== "string") {
        throw new TParserError("isKeyword expects a string parameter.");
    }
    if (keyWords.includes(input.toUpperCase()) || ops.includes(input.toUpperCase()) || logic.includes(input.toUpperCase())) {
        return true;
    }
